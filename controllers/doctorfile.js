...
function Doctorfile() {
    ...
    */
    this.save = function(req, res){
        var doctorfile = models.doctorfile.build(req);
        var errors = [];
        // an example using an object instead of an array
        async.parallel([
            function (callback) {
                doctorfile.validate().then(function (err) {
                    if (err !== null) {
            ...
            if (uniqueError.length === 0) {
                if (typeof req.id !== 'undefined' && req.id !== '') {
                    models.doctorfile.update(req,{where: {id:req.id}, individualHooks: true}).then(function(data){
                        res({status:true, message:language.lang({key:"addedSuccessfully", lang:req.lang}), data:data});
                    }).catch(() => res({status:false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
                } else {
                    models.doctorfile.create(req, {individualHooks: true}).then(function(hospitalfiles){
                        doctor.updateProfileStatusWhileUpdate({id: req.doctorProfileId, langId: req.langId}, function(resp) {
                            if(resp.status) {
    ...
    this.status = function(req, res) {
        models.doctorfile.update(req,{where:{id:req.id}}).then(function(data){
