 ...
 *
 */
router.post('/tagtypes', authorise, formData, (req, res) => {
	//req.roleAccess = {model:'tagtype', action:'view'};
	//auth.checkPermissions(req, isPermission => {
		//if (isPermission.status === true) {
			tagtype.getAll(req.body, result => res.send(result));
		//} else {
		////	res.send(isPermission);
		//}
	//});
 ...
 *
 */
router.post('/tagsbyType', formData, (req, res) => {
//	/req.roleAccess = {model:'patient', action:'add'};
	//auth.checkPermissions(req, isPermission => {
	//	if (isPermission.status === true) {
			req.where = {'$or': [{userId: req.body.userId, is_active: 1}, {is_active: 1, is_approved: 1}]};
