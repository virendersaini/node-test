var hospitalfile = require('../../controllers/hospitalfile');
    ...
    uploadFile(req, res, function (err) {
      errors=[];
        if (err) {
            ...
            return res.send({status: false,errors:[{path:'hospital_files',message:err}], message: err, data: []});
        }
        var data = req.body;
            ...
            if (isPermission.status === true) {
                async.forEach(req.files, function (up_files, callback) {
                  ...
                  count++;
                    ...
                    hospitalfile.save(data, function(result){
                        res.send(result);
                    });
                });
            } else {
