...
module.exports = function(sequelize, DataTypes) {
    var Model = sequelize.define('healthcareprofile', {
        id: {
        ...
        classMethods : {
            ...
            mongo_update:function(reqData,res){
                ...
                where.doctorprofiledetail = language.buildLanguageQuery(
                    where.doctorprofiledetail, req.langId, '`doctorprofile`.`id`', sequelize.models.doctorprofiledetail, 'doctorProfileId'
                );
                Model.hasMany(sequelize.models.doctorprofiledetail);
                Model.find({
                    attributes: ['id','doctor_profile_pic', 'doctorprofiledetails.name', 'is_live','is_active'],
                    include: [{
                    ...
                    Model.checkLiveHospital({
                        ...
                        var save_json_data_in_mongodb = {
                            ...
                            langId: resultData.doctorprofiledetails[0].languageId,
        ...
        filterDoctors: function (req, res) {
            if (typeof req !== 'undefined') {
                ...
                // if(!_.isEmpty(req.mobile)){
                //     cityCond['mobile'] = { $like : '%' + req.mobile + '%' };
                // }

                // if(!_.isEmpty(req.email)){
                //     cityCond['email'] = { $like : '%' + req.email + '%' };
                // }
